...
const DashboardFirebaseContext =
  createContext<DashboardFirebaseValue>(defaultValue);
...
export function computeAggregatedStats(
  ...
  for (const entry of Object.values(stats)) {
    if (entry && typeof entry === "object") {
      ...
      aggregated.sendText += Number(entry.sendText ?? 0);
      for (const [key, count] of Object.entries(entry.sounds ?? {})) {
        aggregated.sounds[key] = (aggregated.sounds[key] ?? 0) + count;
        if (aggregated.sounds[key] > aggregated.maxSound)
          aggregated.maxSound = aggregated.sounds[key];
      }
      for (const [key, count] of Object.entries(entry.texts ?? {})) {
        aggregated.texts[key] = (aggregated.texts[key] ?? 0) + count;
        if (aggregated.texts[key] > aggregated.maxText)
          aggregated.maxText = aggregated.texts[key];
      }
    }
  }
  return aggregated;
...
export function DashboardFirebaseProvider({
  value,
  children,
}: DashboardFirebaseProviderProps) {
  return (
