...
var wsHost =
  location.hostname && location.hostname !== ""
    ...
    : "localhost:8000";
var wsUrl =
  (location.protocol === "https:" ? "wss:" : "ws:") + "//" + wsHost + "/ws";
...
function initDebugPanel() {
  ...
  document
    .getElementById("debug-btn-close")
    .addEventListener("click", function () {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "`") {
      ...
      document.getElementById("debug-toggle").style.display = hidden
        ? "block"
        : "none";
    }
  });
...
function dbg(kind, text) {
  ...
  var prefix =
    { sent: "▶ SEND", recv: "◀ RECV", info: "ℹ INFO", error: "✖ ERR " }[kind] ||
    kind;
  ...
  if (pageId === "soundboard") {
    document.getElementById("conversation-title").textContent =
      app.connectedPeerName
        ? app.connectedPeerName + "'s Conversation"
        : "Conversation";
    renderMessages();
  }
}
...
function renderGeneralInfo() {
  ...
  document.getElementById("general-car").textContent =
    [info.model, info.color].filter(Boolean).join(" ") || "—";
...
function renderIncomingRequests() {
  ...
  if (!app.incomingRequests.length) {
    container.innerHTML =
      '<div class="request-item"><span style="color:#888">No pending requests</span></div>';
    return;
  }
  container.innerHTML = app.incomingRequests
    .map(function (r) {
      var label = r.plate || r.car_id;
      return (
        ...
        '<button type="button" class="btn-deny"   title="Reject" data-car-id="' +
        r.car_id +
        '">✕</button>' +
        '<button type="button" class="btn-accept" title="Accept" data-car-id="' +
        r.car_id +
        '">✓</button>' +
        "</div></div>"
      );
  ...
  container.querySelectorAll(".btn-accept").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      app.incomingRequests = app.incomingRequests.filter(function (r) {
        return r.car_id !== carId;
      });
      var peer =
        app.nearby.find(function (c) {
          return c.car_id === carId;
        }) || {};
...
function renderNearby() {
  ...
  if (!app.nearby.length) {
    container.innerHTML =
      '<div class="connection-item"><span style="color:#888">No cars nearby — try scanning</span></div>';
    return;
  }
  container.innerHTML = app.nearby
    .map(function (c) {
      var label = c.plate || c.car_id;
      return (
        '<div class="connection-item">' +
        "<span>" +
        label +
        "</span>" +
        '<button type="button" class="btn-link" title="Connect" data-car-id="' +
        c.car_id +
        '">⎋</button>' +
        "</div>"
      );
...
function onMessage(ev) {
  ...
  switch (t) {
    ...
    case "carInfoUpdated": {
      var cid = p.car_id;
      var idx = app.nearby.findIndex(function (c) {
        return c.car_id === cid;
      });
      var entry = {
        car_id: cid,
    ...
    case "connectionAccepted": {
      ...
      app.state = "connected";
      var peer = app.nearby.find(function (c) {
        return c.car_id === p.car_id;
      });
      app.connectedPeerName = peer
        ? peer.owner || peer.plate || p.car_id
        : p.car_id;
      showPage("soundboard");
      break;
    }
...
function allFieldsFilled() {
  return Array.from(formInputs).every(function (input) {
...
btnConfirm.addEventListener("click", function () {
  ...
  function doSubmit() {
    if (ws && ws.readyState === WebSocket.OPEN) {
      send({
        type: "changeInfo",
        payload: { owner: owner, model: model, color: color, plate: plate },
      });
      showPage("control");
      ...
      dbg(
        "info",
        "Waiting for WS to open before submitting… (readyState=" +
...
document
  .getElementById("btn-refresh-nearby")
  .addEventListener("click", function () {
    send({ type: "getNearby", payload: {} });
